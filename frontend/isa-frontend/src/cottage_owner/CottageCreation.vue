    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                <input v-model="cottage.name" class="form-control" type="text">
                <br>
                
                <h5 class="text-start">Description: </h5>
                <textarea class="form-control" v-model="cottage.description" rows="3"></textarea>
                <br>

                
                <h5 class="text-start">Behaviour: </h5>
                <textarea class="form-control" v-model="cottage.behaviourRules" rows="2"></textarea> <br>

                <div class="row">
                    <div class="col-sm-7"><h5 class="text-start">Max number of people: </h5></div>
                    <div class="col-sm-5 d-flex"><input class="form-control" v-model="cottage.capacity" style="width: 60px;" type="number" min=0></div>
                </div>

                <div class="row">
                    <div class="col-sm-7"><h5 class="text-start">Reservation Cancellation Conditions: </h5></div>
                    <div class="col-sm-5 d-flex">
                        <select v-model="cottage.reservationCancellationConditions" class="form-select cenaCss">
                ...
                <div v-if="cottage.additionalEquipments.length > 0" class="card">
                  <div class="card-body">
                      <span v-for="addEq in cottage.additionalEquipments" :key="addEq" class="badge bg-primary m-1">{{addEq.name}}</span> 
                  </div>
                </div>
                        ...
                        <div class="card-body">
                          <div class="row">
                            <div class="col cenaCss">
                              <h2><b>$<input class="cenaCss" style="width: 120px;" type="number" min=0 v-model="cottage.price.price"></b></h2>
                              <h6><b>
                              <select v-model="cottage.price.priceType" class="form-select cenaCss" aria-label="Default select example" >
              ...
              <div class="card-body">
                  <div style="height: 400px;" class="row">
                      ...
                      <div v-if="cottage.address != undefined" class="col-sm-3">
                          <h6 class="card-title text-start">Country: <input type="text" v-model="cottage.address.country"></h6>
                          <h6 class="card-title text-start">City: <input type="text" v-model="cottage.address.city"></h6>
                          <h6 class="card-title text-start">Street: <input type="text" v-model="cottage.address.street"> </h6>
                          <h6 class="card-title text-start">Number: <input type="text" v-model="cottage.address.number"></h6>
                      </div>
                  </div>
              </div>         
            </div>
        ...
        <br> <br>
        <button v-on:click="createNewCottage()" class="btn btn-primary m-5 p-3">Create new cottage</button>
...
this.map = new Map({
  ...
  methods: {
    ...
    clickAdditionalEq: function(additionalEq){
        for (var i = 0; i < this.cottage.additionalEquipments.length; i++) {
            if(this.cottage.additionalEquipments[i].id == additionalEq.id){
                //remove
                this.cottage.additionalEquipments.splice(i, 1);
        ...
        //add
        this.cottage.additionalEquipments.push(additionalEq);
    ...
    clickItemPrice: function(itemPrice){
        for (var i = 0; i < this.cottage.pricelist.length; i++) {
            if(this.cottage.pricelist[i].id == itemPrice.id){
                //remove
                this.cottage.pricelist.splice(i, 1);
                console.log("removed element from itemPrice with id="+itemPrice.id);
                return;
            }
        }
        //add
        this.cottage.pricelist.push(itemPrice);
    ...
    createNewCottage: function(){
        console.log('NEW Cottage')
        this.cottage.imagesForBackend = this.imagesForBackend;
        // console.log(this.cottage)

        this.cottage.address.longitude = this.map.newLon;
        this.cottage.address.latitude = this.map.newLat;
        // console.log(this.cottage)

        axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
        axios.post('http://localhost:8180/api/cottages/addnewcottage',this.cottage)
          .then(response => {
              console.log('Odgoovr');
              console.log(response.data);
              alert("You created new cottage")
          }).catch(err => {
