...
export default {
  name: 'CottageReservationViewComponent',
  props: ['reservation'],
  components: {
      ClientViewComponent,
      Datepicker
  ...
  methods: {
    ...
    moreDetails: function(adventureName){
        this.$router.push({ path: '/cottage/'+encodeURIComponent(adventureName)});
    },
    ...
    sendReport: function(){
        if(new Date(this.reservation.endTime) < new Date()){
            ...
            axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
            axios.post('http://localhost:8180/api/report/cottageowner',instructorReport).then(resp => {
                    console.log(resp.data);
                    if(!resp.data){
                        alert("Error")
                    }else{
    ...
    addItemPriceFastReservation: function (itPrice) {  
      ...
      axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
      axios.post('http://localhost:8180/api/cottage-terms/reserve-again', newFastReservationDTO).then(
        (resp) => {
            console.log(resp.data)
            alert(resp.data)
