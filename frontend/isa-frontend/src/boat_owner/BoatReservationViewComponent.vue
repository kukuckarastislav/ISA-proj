...
<div class="card" style="background-color: whitesmoke;">
    <div class="card-body">
        <div class="row">
            ...
            <!-- Info -->
            <div class="col-3">
                <h5 class="card-title text-start"><a v-on:click="moreDetails(reservation.boatName)" href="">{{reservation.boatName}}</a></h5>
                <br>
        ...
        <div v-if="visibleForm.complaint" class="row" style="margin-top:30px;">
            <h4>Complaint</h4>
            <div class="col" v-if="!reservation.ownerComplaint">
                <textarea v-model="complaintText" style="width: 100%;" name="complaint" id="" rows="4"></textarea>
            </div>
            <div v-if="!reservation.ownerComplaint" class="col-3">
            ...
            <div class="col" v-if="reservation.ownerComplaint">
                <div class="card">
                    <div class="card-body">
                        <p class="card-text text-start">For client: {{reservation.ownerComplaint.userEmail}} At: {{convertDate(reservation.ownerComplaint.createdAt)}}</p>
                        <p class="card-text text-start">Your Complaint: {{reservation.ownerComplaint.comment}}</p>
                        <div v-if="reservation.ownerComplaint.statusOfComplaint === 'ANSWERED'">
                            <hr>
                            <p class="card-text text-start"><b>ANSWERED At: {{convertDate(reservation.ownerComplaint.adminResponsDate)}}</b> By: {{reservation.ownerComplaint.adminEmail}}</p>
                            <p class="card-text text-start">{{reservation.ownerComplaint.adminResponse}}</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
  name: 'BoatReservationViewComponent',
  props: ['reservation'],
  components: {
      ClientViewComponent,
      Datepicker
  ...
  methods: {
    ...
    moreDetails: function(adventureName){
        this.$router.push({ path: '/boat/'+encodeURIComponent(adventureName)});
    },
    ...
    sendComplaint: function(){
        if(new Date(this.reservation.endTime) < new Date()){
            ...
            axios.post('http://localhost:8180/api/complaint/boatowner',instructorComplaint).then(resp => {
    ...
    sendReport: function(){
        if(new Date(this.reservation.endTime) < new Date()){
            ...
            axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
            axios.post('http://localhost:8180/api/report/boat',instructorReport).then(resp => {
                    console.log(resp.data);
                    if(!resp.data){
                        alert("Error")
                    }else{
    ...
    addItemPriceFastReservation: function (itPrice) {  
      ...
      axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
      axios.post('http://localhost:8180/api/boat-terms/reserve-again', newFastReservationDTO).then(
        (resp) => {
            console.log(resp.data)
            alert(resp.data)
