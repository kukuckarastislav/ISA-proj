  ...
  methods: {
      loadInformation: function(){
          axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
            axios.get('http://localhost:8180/api/admin/profileInfo')
      ...
      saveEditProfile: function(){
          this.isEditingProfile = false;
          axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
            axios.put('http://localhost:8180/api/admin/updateProfile',this.profile).then(response => 
      ...
      saveEditPassword: function(){
          this.isEditingPassword = false;
          axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
          axios.put('http://localhost:8180/api/admin/updatePassword',this.passwordData)
          .then(response => {
                  if (response.data) {
      ...
      sendDeleteAccReq: function(){
          this.isDeletingAcc = false;
          axios.defaults.headers.common["Authorization"] = "Bearer " + window.sessionStorage.getItem("jwt");  
          axios.put('http://localhost:8180/api/admin/deleteRequest',{reason: this.deleteRequestMessage, username: this.profile.email})
          .then(response => {
                  if (response) {
                        window.sessionStorage.clear();
                        this.$router.push('/');
